...
class ClientGenerator {
  ...
  String generate() {
    ...
    buf.writeln(
        '  StreamSubscription<IdentityTokenMessage>? _identityTokenSub;');
    buf.writeln();
    ...
    for (final table in schema.tables) {
      final className = _toPascalCase(table.name);
      buf.writeln(
          "    subscriptionManager.cache.registerDecoder<$className>('${table.name}', ${className}Decoder());");
    }
    buf.writeln();
    ...
    buf.writeln(
        "        SdkLogger.w('Connection failed, operating in offline mode: \$e');");
    buf.writeln('      } else {');
    buf.writeln('        rethrow;');
    buf.writeln('      }');
    buf.writeln('    }');
    ...
    buf.writeln();
    buf.writeln(
        '  /// Dispose all resources - cancel listeners, dispose subscription manager, close connection.');
    buf.writeln('  ///');
    buf.writeln(
        '  /// Call this when the client is no longer needed (e.g., logout, app shutdown).');
    buf.writeln(
        '  /// After dispose, the client instance should not be reused.');
    buf.writeln('  Future<void> dispose() async {');
    buf.writeln('    _identityTokenSub?.cancel();');
    buf.writeln('    _identityTokenSub = null;');
    buf.writeln('    subscriptions.dispose();');
    buf.writeln('    await connection.disconnect();');
    buf.writeln('  }');
    buf.writeln();
